...
import { getTaksByUserID, taskRegisterAPI, updateTaksAPI } from '../../services/tasks'
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getTask.fulfilled, (state, action) => {
        state.loading = false
        if (
          action.payload.httpStatus === 200 && action.payload.content.length > 0
        ) {
      ...
      .addCase(registerTask.fulfilled, (state, action) => {
        state.loading = false
        if (
          action.payload.httpStatus === 201 && action.payload.status === "success"
        ) {
          state.httpStatus = action.payload.httpStatus
          state.status = action.payload.status
      ...
      .addCase(updateTask.fulfilled, (state, action) => {
        state.loading = false
        if (
          action.payload.httpStatus === 200 && action.payload.status === "success"
        ) {
          state.httpStatus = action.payload.httpStatus
          state.status = action.payload.status
          const index = state.content.findIndex(task => task._id === action.payload.updated._id);
